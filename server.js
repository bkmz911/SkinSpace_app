...
//         failureRedirect: "/",
//         failureMessage: true, // покажет причину в сессии
//     }),
//     (req, res) => {
    ...
    new OpenIDStrategy(
        {
            ...
            returnURL: "http://localhost:3001/auth/steam/return",
            realm: "http://localhost:3001/",
            stateless: true, // важно для Steam
        },
        async (identifier, done) => {
            try {
                // Извлекаем SteamID64 из identifier
                const steamId = identifier.match(/(\d+)$/)?.[1];
                console.log("steamId:", steamId);
                console.log("API KEY:", process.env.STEAM_API_KEY);
                if (!steamId) {
                    return done(new Error("Не удалось извлечь SteamID"));
                }
                ...
                if (!players || players.length === 0) {
                    return done(new Error("Профиль Steam не найден"));
                }

                const profile = players[0];
                profile.id = steamId; // добавляем id чтобы остальной код не менять
...
app.get("/auth/steam", passport.authenticate("openid"));
